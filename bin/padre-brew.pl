...
if ( $sysname eq 'Darwin' ) {
    $opt->{_platform} = 'osxsnowleopard';
    if ($release =~ /^11/) { 
        # Lion
        $opt->{_platform} = 'osxlion';
        $ENV{PERL_MB_OPT} = '--wxWidgets-extraflags=" --with-macosx-sdk=/Developer/SDKs/MacOSX10.6.sdk --with-macosx-version-min=10.5 " --wxWidgets-build=\'yes\' --wxWidgets-source=\'tar.bz2\' --wxWidgets-version=2.8.12';
